...
const App = () => {
    ...
    // Sample Test Data (Simulated from backend)
    const [testData, setTestData] = useState({
        "Psychiatry": [
            { title: "Neurocognitive, Mood, Subs related, Neurosis 1", mcqs: 50, date: "2024-06-25" },
            { title: "Neurocognitive, Mood, Subs related, Neurosis 2", mcqs: 50, date: "2024-06-25" },
            { title: "Personality, Specific, Psychiatric emergency and Clinical psychiatry 1", mcqs: 50, date: "2024-06-25" },
            { title: "Child Psychiatry & Forensics", mcqs: 40, date: "2024-06-24" },
            { title: "Schizophrenia & Other Psychotic Disorders", mcqs: 60, date: "2024-06-23" },
            { title: "Anxiety & Somatoform Disorders", mcqs: 45, date: "2024-06-22" }
        ],
        "Surgery": [
            { title: "General Surgery Principles", mcqs: 100, date: "2024-06-20" },
            { title: "Abdominal Surgery MCQs", mcqs: 75, date: "2024-06-19" },
            { title: "Orthopedic Surgery Basics", mcqs: 80, date: "2024-06-18" },
            { title: "Cardiothoracic Surgery Test", mcqs: 70, date: "2024-06-17" },
            { title: "Vascular Surgery Quiz", mcqs: 50, date: "2024-06-16" },
            { title: "Pediatric Surgery Scenarios", mcqs: 60, date: "2024-06-15" }
        ],
        "Obs&Gyne": [
            { title: "Obstetrics Basics", mcqs: 60, date: "2024-06-10" },
            { title: "Gynecology Disorders", mcqs: 70, date: "2024-06-09" },
            { title: "Antenatal Care & Complications", mcqs: 55, date: "2024-06-08" },
            { title: "Labor & Delivery Management", mcqs: 65, date: "2024-06-07" },
            { title: "Reproductive Endocrinology", mcqs: 40, date: "2024-06-06" }
        ]
    });

    const [todayFocusItems, setTodayFocusItems] = useState([
        {
            id: "Psychiatry-Sexuality, Child Psychi, Foreign Pshci, Facets and Psychotic disorders 1",
            subject: "Psychiatry",
            title: "Sexuality, Child Psychi, Foreign Pshci, Facets and Psychotic disorders 1",
            mcqs: 50,
            type: "Custom",
        ...
        {
            id: "Psychiatry-Sexual, Psychotic & Personality disorders",
            subject: "Psychiatry",
            title: "Sexual, Psychotic & Personality disorders",
            mcqs: 50,
            type: "Custom",
            iconPath: "M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm13 14v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"
        }
    ...
    const [timetableEntries, setTimetableEntries] = useState([
        { id: 't1', subject: 'Anatomy', topic: 'Upper Limb', date: '2024-07-01', time: '09:00' },
        { id: 't2', subject: 'Physiology', topic: 'Cardiac Cycle', date: '2024-07-01', time: '11:00' },
        { id: 't3', subject: 'Pharmacology', topic: 'Antibiotics', date: '2024-07-02', time: '14:00' },
    ]);

    const [currentSelectedSubject, setCurrentSelectedSubject] = useState('Psychiatry'); // For TestsCompletedPage
    const [currentTimetableSubject, setCurrentTimetableSubject] = useState('Psychiatry'); // For TimetablePage progress
    ...
    // --- Data Manipulation Callbacks (passed to children) ---
    ...
    const addTest = useCallback((subjectName, title, mcqs, date) => {
        if (!subjectName || !title || !mcqs || !date) {
            showNotification('All test fields are required.', 'error');
            return;
        }
        if (!testData.hasOwnProperty(subjectName)) {
            showNotification('Subject does not exist.', 'error');
            return;
        }
        ...
        setTestData(prevData => ({
            ...prevData,
            [subjectName]: [...prevData[subjectName], newTest]
        }));
    ...
    const deleteSubject = useCallback((subjectName) => {
        if (testData.hasOwnProperty(subjectName)) {
            ...
            // Adjust current selected subjects if the deleted one was active
            if (currentSelectedSubject === subjectName) {
                setCurrentSelectedSubject(Object.keys(updatedTestData).length > 0 ? Object.keys(updatedTestData)[0] : '');
            }
            if (currentTimetableSubject === subjectName) {
                setCurrentTimetableSubject(Object.keys(updatedTestData).length > 0 ? Object.keys(updatedTestData)[0] : '');
            }
            showNotification(`Subject "${subjectName}" deleted.`, 'success');
        }
    }, [testData, currentSelectedSubject, currentTimetableSubject, showNotification]);

    const deleteTest = useCallback((subjectName, testTitle) => {
        if (testData.hasOwnProperty(subjectName)) {
            const initialLength = testData[subjectName].length;
            const updatedTests = testData[subjectName].filter(test => test.title !== testTitle);
            if (updatedTests.length < initialLength) {
                setTestData(prevData => ({
                    ...prevData,
    ...
    const addTimetableEntry = useCallback((subject, topic, date, time) => {
        ...
        const newId = `t${timetableEntries.length + 1}-${Date.now()}`;
        setTimetableEntries(prevEntries => [...prevEntries, { id: newId, subject, topic, date, time }]);
        ...
        if (notificationPermission === 'granted') {
            ...
            // Schedule notification a bit before the event, e.g., 5 minutes
            ...
            if (reminderTime > 0) {
                setTimeout(() => {
                    new Notification('Upcoming Study Event', {
                        ...
                        icon: 'https://placehold.co/48x48/007bff/ffffff?text=🔔' // Placeholder icon
                    });
                }, reminderTime);
            } else {
    ...
    }, [timetableEntries, showNotification, notificationPermission]);

    const deleteTimetableEntry = useCallback((idToDelete) => {
    ...
    const navigateTo = useCallback((pageName, options = {}) => {
        ...
        // Special handling for specific pages, similar to original JS
        ...
        switch (currentPage) {
            ...
            case 'manage-tests':
                return <ManageContentPage
                        ...
                        />;
            case 'profile':
                return <ProfilePage onManageContent={() => navigateTo('manage-tests')} onLogout={handleLogout} />;
            default:
                return <LoginPage onLogin={handleLogin} />;
        }
    };
    ...
    return (
        <div className="max-w-md mx-auto bg-gray-50 min-h-screen shadow-lg relative overflow-hidden">
            {!isLoggedIn ? (
                renderPage()
            ) : (
                <>
...
const TimetableEventCard = ({ event, isEditMode, onDelete }) => {
                ...
                {timetableEntries.length === 0 ? (
                    ...
                    sortedDates.map(date => (
                        <React.Fragment key={date}>
                            ...
                            <TimetableEventCard event={groupedEntries[date]} isEditMode={timetableEditMode} onDelete={deleteTimetableEntry} />
                        </React.Fragment>
                    ))
                )}
            </div>
...
const ProfilePage = ({ onManageContent, onLogout }) => {
    return (
